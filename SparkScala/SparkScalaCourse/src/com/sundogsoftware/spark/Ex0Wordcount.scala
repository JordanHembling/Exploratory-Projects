...
object Ex0Wordcount {
  ...
  def loadData(): RDD[String] = {
    ...
    // It represents the connection to Spark and it is the place where you can configure the common properties
    // like the app name, the master url, memories allocation...
    val sc = new SparkContext("local[*]", "Ex0Wordcount")
  ...
  def wordcount(): RDD[(String, Int)] = {
    ...
    // TODO write code here
    val reducedPairs = RDDpairs.reduceByKey((x,y) => x + y) 
    (reducedPairs)
   }
  ...
  def filterOnWordcount(): RDD[(String, Int)] = {
    ...
    val commonTweets = tweets.filter(x => x._2 > 4)
    (commonTweets)
  }
  
  def main(args: Array[String]) {
    Logger.getLogger("org").setLevel(Level.ERROR)
    val results = filterOnWordcount()
    val answer = results.collect()
